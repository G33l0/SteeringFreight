...
/**
 * Country list used by the quote form, the shipment screens and the customer
 * records, so origins and destinations are recorded consistently instead of
 * being typed differently every time.
 *
 * The list is deliberately complete rather than a shortlist of lanes: a
 * forwarder is asked to quote for places it has never shipped to, and the
 * business should not have to edit code to accept the enquiry.
 */
class Countries
{
    /**
     * @return array<string, string> ISO 3166-1 alpha-2 code => country name
     */
    public static function all(): array
    {
        return [
            ...
            'GI' => 'Gibraltar', 'GR' => 'Greece', 'GD' => 'Grenada', 'GT' => 'Guatemala',
            ...
            'MO' => 'Macao SAR China', 'MG' => 'Madagascar', 'MW' => 'Malawi', 'MY' => 'Malaysia',
            ...
            'PK' => 'Pakistan', 'PS' => 'Palestinian Territories', 'PA' => 'Panama',
            ...
            'TN' => 'Tunisia', 'TR' => 'Türkiye', 'TM' => 'Turkmenistan', 'UG' => 'Uganda',
    ...
    /**
     * Names only, alphabetically, for a plain select or datalist.
     *
     * @return list<string>
     */
     ...
     * @return list<string>
     */
    public static function frequentlyUsed(): array
    {
        $configured = collect(app(Settings::class)->list('quotes.frequent_countries'))
            ->map(fn (array $row) => trim($row['title'] ?? ''))
            ->filter(fn (string $name) => self::isValidName($name))
            ->values()
            ->all();

        return $configured !== [] ? $configured : [
            'China', 'India', 'Singapore', 'United Arab Emirates', 'Türkiye',
            'Netherlands', 'United Kingdom', 'United States', 'Nigeria', 'Ghana',
        ];
    }
}
